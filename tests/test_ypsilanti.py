# tests/test_ypsilanti.py
from pathlib import Path
from unittest.mock import patch, MagicMock
from scrapers.base import is_in_window

FIXTURE = (Path(__file__).parent / "fixtures" / "ypsilanti.html").read_text(encoding="utf-8")
...
def test_scrape_returns_list():
    with patch("scrapers.ypsilanti.RateLimitedSession") as cls:
        cls.return_value = _mock_session(FIXTURE)
        from scrapers.ypsilanti import scrape
...
def test_events_source_and_category():
    with patch("scrapers.ypsilanti.RateLimitedSession") as cls:
        ...
        from scrapers.ypsilanti import scrape
        for e in scrape():
            assert e["source"] == "ypsilanti"
            assert e["category"] == "ypsilanti"
